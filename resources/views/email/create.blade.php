    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">New Global Campaign</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Send to the global subscriber list.</p>
                </div>
                <a href="{{ route('email.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Back to Email</a>
            </div>

            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('email.store') }}" class="p-4 sm:p-6 space-y-6">
                    @csrf
                    ...
                    @if ($templates->isNotEmpty())
                        <div>
                            ...
                            <select id="template_id"
                                ...
                                onchange="if (this.value) { window.location = '{{ route('email.create') }}' + '?template_id=' + this.value; }">
