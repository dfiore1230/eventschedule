...
This repository provides a Docker-based runtime for the Planify Laravel application. It packages PHP-FPM, Nginx, MariaDB, and a scheduler worker so that the app can be bootstrapped quickly for local development, testing, or small-scale deployments.
...
| `db`       | MariaDB 11 database with credentials controlled by `.env.docker`.           |
...
- **Database access**: Connect to MariaDB on `localhost:3306` (when exposed) using credentials defined in `.env.docker`.
