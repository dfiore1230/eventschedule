...
class EventRepo
{
    ...
    {
        $mode = $event && $event->exists ? 'update' : 'create';
        ...
        $this->writeLog('info', 'EventRepo.saveEvent.start', $context + ['input_summary' => $inputSummary]);
        ...
        try {
            ...
            if ($request->venue_address1) {
                if (! $venue) {
                    $this->writeLog('debug', 'EventRepo.saveEvent.creatingVenue', $context + ['venue_email' => $request->venue_email]);
                    ...
                    $this->writeLog('debug', 'EventRepo.saveEvent.updatedVenue', $context + ['venue_id' => $venue->id]);
                }
            }
            ...
            if (! $event) {
                $this->writeLog('debug', 'EventRepo.saveEvent.creatingEvent', $context + ['venue_id' => $venueId]);
            ...
            */
            ...
            $this->writeLog('debug', 'EventRepo.saveEvent.persistingEvent', $context + ['changes' => Arr::except($event->getDirty(), ['updated_at'])]);
            ...
            if ($request->hasFile('flyer_image')) {
                ...
                $this->writeLog('debug', 'EventRepo.saveEvent.flyerUploaded', $context + ['flyer_path' => $filename]);
                ...
                if ($asset) {
                    ...
                    $this->writeLog('debug', 'EventRepo.saveEvent.flyerLinked', $context + ['asset_id' => $asset->id, 'variant_id' => $variant?->id]);
                }
            }
            ...
            if ($event->tickets_enabled) {
                ...
                $this->writeLog('debug', 'EventRepo.saveEvent.ticketsProcessed', $context + ['ticket_ids' => $ticketIds]);
            } else {
                $event->tickets()->update(['is_deleted' => true]);
                $this->writeLog('debug', 'EventRepo.saveEvent.ticketsDisabled', $context);
            }
            ...
            $this->writeLog('info', 'EventRepo.saveEvent.success', $context + [
                'event_id' => $event->id,
            ...
            $this->writeLog('error', 'EventRepo.saveEvent.failed', $context + [
    ...
    private function writeLog(string $level, string $message, array $context = []): void
    {
        try {
            ...
            error_log(sprintf(
                'EventRepo logging failure for %s: %s',
                $message,
